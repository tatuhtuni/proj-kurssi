...
def test_whatever():
    ...
    assert (warning_msg.find(warning_msg) != -1 and
            warning_msg.find(warning_name) != -1)
...
{VT100_UNDERLINE}WHERE type 'C'{VT100_RESET} = 1 OR 100 = 100;"""
    ...
    warning_msg = formatter.format()
    assert (warning_msg.find(warning) != -1 and
            warning_msg.find(warning_name) != -1 and
            warning_msg.find(underlined_query) != -1)
