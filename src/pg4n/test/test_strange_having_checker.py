...
FROM {ORDERS_TABLE_NAME}
GROUP BY customer_id
HAVING SUM(order_total_eur) > 150;"""

    sql_statement = SQL_NEITHER_GROUP_BY_OR_HAVING
    parsed_sql = sql_parser.parse_one(sql_statement)
    qep_analysis = qep_parser.parse(sql_statement)
    checker = StrangeHavingChecker(parsed_sql, qep_analysis)
    assert checker is not None
    warning_msg = checker.check()
    assert warning_msg is None

    sql_statement = SQL_GROUP_BY_WITHOUT_HAVING
    parsed_sql = sql_parser.parse_one(sql_statement)
    qep_analysis = qep_parser.parse(sql_statement)
    checker = StrangeHavingChecker(parsed_sql, qep_analysis)
    assert checker is not None
    ...
    sql_statement = SQL_HAVING_WITHOUT_GROUP_BY
    parsed_sql = sql_parser.parse_one(sql_statement)
    qep_analysis = qep_parser.parse(sql_statement)
    checker = StrangeHavingChecker(parsed_sql, qep_analysis)
    assert checker is not None
    ...
    sql_statement = SQL_GROUP_BY_WITH_HAVING
    parsed_sql = sql_parser.parse_one(sql_statement)
    qep_analysis = qep_parser.parse(sql_statement)
    checker = StrangeHavingChecker(parsed_sql, qep_analysis)
    assert checker is not None
