...
FROM {ORDERS_TABLE_NAME};"""
    ...
    qep_analysis = qep_parser.parse(sql_statement)
    checker = SumDistinctChecker(parsed_sql, qep_analysis)
    assert checker is not None
    warning_msg = checker.check()
    assert warning_msg is None

    print("SQL_SUM_WITH_DISTINCT")
    sql_statement = SQL_SUM_WITH_DISTINCT
    parsed_sql = sql_parser.parse_one(sql_statement)
    qep_analysis = qep_parser.parse(sql_statement)
    checker = SumDistinctChecker(parsed_sql, qep_analysis)
    assert checker is not None
    ...
    print("SQL_SUM_WITHOUT_DISTINCT")
    sql_statement = SQL_SUM_WITHOUT_DISTINCT
    parsed_sql = sql_parser.parse_one(sql_statement)
    qep_analysis = qep_parser.parse(sql_statement)
    checker = SumDistinctChecker(parsed_sql, qep_analysis)
    assert checker is not None
    ...
    print("SQL_AVG_WITH_DISTINCT")
    sql_statement = SQL_AVG_WITH_DISTINCT
    parsed_sql = sql_parser.parse_one(sql_statement)
    qep_analysis = qep_parser.parse(sql_statement)
    checker = SumDistinctChecker(parsed_sql, qep_analysis)
    assert checker is not None
    ...
    print("SQL_AVG_WITHOUT_DISTINCT")
    sql_statement = SQL_AVG_WITHOUT_DISTINCT
    parsed_sql = sql_parser.parse_one(sql_statement)
    qep_analysis = qep_parser.parse(sql_statement)
    checker = SumDistinctChecker(parsed_sql, qep_analysis)
    assert checker is not None
