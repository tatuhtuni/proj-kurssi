...
from ..nested_condition_checker import NestedConditionChecker
...
def test_check(sql_parser: SqlParser):
    SQL_NESTED_CONDITION_SUSPICIOUS = \
f"""
SELECT order_id
...
WHERE EXISTS (SELECT order_total_eur
    ...
    SQL_NESTED_CONDITION_WITH_TOPLEVEL_CONDITIONS_SUSPICIOUS = \
f"""
SELECT order_id
FROM  {ORDERS_TABLE_NAME}
WHERE EXISTS (SELECT order_total_eur
...
SELECT (1, 2, 3);"""
    ...
    sql_statement = SQL_NESTED_CONDITION_SUSPICIOUS
    parsed_sql = sql_parser.parse_one(sql_statement)
    checker = NestedConditionChecker(parsed_sql)
    assert checker != None
    assert checker.check() == WARNING

    sql_statement = SQL_NESTED_CONDITION_WITH_TOPLEVEL_CONDITIONS_SUSPICIOUS
    parsed_sql = sql_parser.parse_one(sql_statement)
    checker = NestedConditionChecker(parsed_sql)
    assert checker != None
    assert checker.check() == WARNING
