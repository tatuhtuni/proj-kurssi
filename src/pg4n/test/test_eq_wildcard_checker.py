...
WHERE fname = 'Amble' AND
    ...
    checker = EqWildcardChecker(parsed_sql, qep_analysis)
    assert checker is not None
    warning_msg = checker.check()
    assert warning_msg is None

    sql_statement = SQL_EQ_NO_WILDCARD
    parsed_sql = sql_parser.parse_one(sql_statement)
    qep_analysis = qep_parser.parse(sql_statement)
    checker = EqWildcardChecker(parsed_sql, qep_analysis)
    assert checker is not None
    ...
    sql_statement = SQL_LIKE_WILDCARD
    parsed_sql = sql_parser.parse_one(sql_statement)
    qep_analysis = qep_parser.parse(sql_statement)
    checker = EqWildcardChecker(parsed_sql, qep_analysis)
    assert checker is not None
    ...
    sql_statement = SQL_LIKE_WILDCARD_AND_EQ_WILDCARD1
    parsed_sql = sql_parser.parse_one(sql_statement)
    qep_analysis = qep_parser.parse(sql_statement)
    checker = EqWildcardChecker(parsed_sql, qep_analysis)
    assert checker is not None
    ...
    sql_statement = SQL_LIKE_WILDCARD_AND_EQ_NO_WILDCARD1
    parsed_sql = sql_parser.parse_one(sql_statement)
    qep_analysis = qep_parser.parse(sql_statement)
    checker = EqWildcardChecker(parsed_sql, qep_analysis)
    assert checker is not None
    ...
    sql_statement = SQL_LIKE_WILDCARD_AND_EQ_NO_WILDCARD2
    parsed_sql = sql_parser.parse_one(sql_statement)
    qep_analysis = qep_parser.parse(sql_statement)
    checker = EqWildcardChecker(parsed_sql, qep_analysis)
    assert checker is not None
